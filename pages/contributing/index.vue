...
onMounted(async () => {
  contributing.value = ""
  try {
    contributing.value = await $fetch('https://raw.githubusercontent.com/Faeq-F/Quokka/refs/heads/main/CONTRIBUTING.md')
    contributing.value = contributing.value.substring(contributing.value.indexOf("\n") + 1)
  } catch (err: unknown) {
    console.log('Error fetching contributing data:', (err as Error).toString())
  } finally {
    loading.value = false
  }
})
